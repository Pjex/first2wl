        ...
        const Region& inDirtyRegion)
{
    ...
    //panjie add for offline buffer
    char value[PROPERTY_VALUE_MAX];
    property_get("debug.sf.hw", value, "0");
    int temp = atoi(value);
    ...
    if (temp == 2) {
        ...
        engine.beginGroup(mColorMatrix, true);
        doComposeSurfaces(hw, dirtyRegion);
        engine.endGroup(1);
    } else {
        mDebugDisableHWC = 0;
        RenderEngine& engine(getRenderEngine());
        engine.beginGroup(mColorMatrix, false);
        doComposeSurfaces(hw, dirtyRegion);
    }
    ...
    //panjie add clear
    if(temp == 2){//panjie why clear in there ,bucause, the buffer has swap ,so we shuold clear it for next time use it
        RenderEngine& engine(getRenderEngine());
        engine.endGroup(2);
    }
}
